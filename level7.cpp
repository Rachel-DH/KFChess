...
void updateBoardTime(vector<vector<string>> &board)
{
    if (active_move.active && current_time >= active_move.arrival_time)
    {
        ...
        board[active_move.to_r][active_move.to_c] = active_move.piece;
...
void executeClick(vector<vector<string>> &board, int &sel_r, int &sel_c, int r, int c)
{
    ...
    char selectedColor = board[sel_r][sel_c][0];
    char selectedPiece = board[sel_r][sel_c][1];

    if (board[r][c] == "." || board[r][c][0] != selectedColor)
    {
        if (isValidMove(board, selectedPiece, sel_r, sel_c, r, c))
        {
