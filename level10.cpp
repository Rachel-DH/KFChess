        ...
        case 'P':
            {
                ...
                if (color == 'w')
                {
                    // בלוח של 4 שורות, שורת ההתחלה היא אך ורק 3. בלוח גדול יותר, זו שורה size() - 2
                    if ((board.size() == 4 && r1 == 3) || (board.size() > 4 && r1 == (int)board.size() - 2))
                        is_start_row = true;
                }
                else
                {
                    // בלוח של 4 שורות, שורת ההתחלה היא אך ורק 0. בלוח גדול יותר, זו שורה 1
...
void updateBoardTime(vector<vector<string>> &board)
{
    if (active_move.active && current_time >= active_move.arrival_time)
    {
        if (board[active_move.to_r][active_move.to_c] != "." && board[active_move.to_r][active_move.to_c][1] == 'K')
        {
            game_over = true;
        }
        ...
        if (final_piece[1] == 'P')
        {
            if ((final_piece[0] == 'w' && active_move.to_r == 0) || 
                (final_piece[0] == 'b' && active_move.to_r == (int)board.size() - 1))
            {
                final_piece[1] = 'Q';
            }
        }
        ...
        board[active_move.to_r][active_move.to_c] = final_piece;
...
void executeClick(vector<vector<string>> &board, int &sel_r, int &sel_c, int r, int c)
{
    ...
    else
    {
        ...
        if (!active_move.active && board[r][c] != "." && board[sel_r][sel_c][0] == board[r][c][0])
        {
            sel_r = r;
            sel_c = c;
        }
